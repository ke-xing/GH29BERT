...
model.to(device)
train_data_path = f'../../data/new/npz/train/train_onehot1.npz'
test_data_path = f'../../data/new/npz/test/test_onehot1.npz'
...
for epoch in range(num_epoches):
    ...
    print(f'train epoch {epoch+1},loss:{running_loss/len(train_set)},acc:{acc/len(train_set)*100},time:{t2-t1}')
    ...
    with torch.no_grad():
        ...
        t2=time.time()
        print(f'test epoch {epoch + 1},loss:{running_loss / len(test_set)},acc:{acc/len(test_set)*100},time:{t2-t1}\n')
...
plt.savefig(f'onehot1.png')

model.to('cpu')
torch.save(model,f'models/down_model_onehot1.pt')